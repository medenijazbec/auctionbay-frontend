...
const AuctionsPage:React.FC = () => {
  ...
  const placeholders = (n:number) => Array.from({ length:n }, (_,i) => (
    <div key={`ph-${i}`} className={`${styles['auction-card']} ${styles['auction-card--placeholder']}`} />
  ));
  const currentCount =
  ...
  return (
    <div className={styles['landing-container']}>
      <header className={styles['top-nav']}>
        <div className={styles['top-nav-left']}>
          <div className={styles['frame-topbar-left']}>
            ...
            <div className={styles['left-pill-container']}>
              <button
                className={`${styles['nav-btn']} ${activeNav === 'auctions' ? styles['nav-btn-active'] : ''}`}
                onClick={() => setActiveNav('auctions')}
              >
                <img src={houseIcon} alt="" /> <span>Auctions</span>
              </button>
              <button
                className={`${styles['nav-btn']} ${activeNav === 'profile' ? styles['nav-btn-active'] : ''}`}
        ...
        <div className={styles['top-nav-right']}>
          <div className={styles['right-pill-container']}>
            ...
            >
              <img src={avatar} alt="Your avatar" className={styles['user-avatar']} />
              {menuOpen && (
                <div className={styles['user-menu']}>
                  <Link to="/profile" className={styles['menu-link']}>
                    <img src={gearIcon} alt="" className={styles['menu-gear-icon']} />
                    Profile settings
                  </Link>
          ...
          <div className={styles['auctions-container']}>
            <div className={styles['auction-grid']}>
              {auctions.map(a => {
                ...
                return (
                  <div key={a.auctionId} className={styles['auction-card']}>
                    <div className={styles['auction-card-header']}>
                      <span className={`${styles['auction-tag']} ${styles[status]}`}>
                        {getTagText(a.auctionState)}
                      </span>
                      <span className={`${styles['time-tag']} ${styles[status]}`}>
                        {getTimeText(a.endDateTime)}
                        <img
                          src={clock15 /* you can pick a generic clock icon here */}
        ...
        <div className={styles['profile-view']}>
          ...
          <div className={styles['profile-tabs']}>
            {(['myAuctions','bidding','won'] as const).map(tab => (
              <button
                key={tab}
                className={`${styles['profile-tab']} ${subTab === tab ? styles['profile-tab--active'] : ''}`}
                onClick={() => setSubTab(tab)}
              >
                {tab === 'myAuctions' ? 'My auctions' : tab === 'bidding' ? 'Bidding' : 'Won'}
              </button>
            ))}
          </div>
          ...
          <div className={styles['profile-content']}>
            {subTab === 'myAuctions' && (
              myAuctions.length ? (
                        ...
                        <div key={a.auctionId} className={styles['auction-card']}>
                          <div className={styles['auction-card-header']}>
                            ...
                            <span className={`${styles['time-tag']} ${styles[status]}`}>
                              ...
                              <img src={getClockIcon(a.endDateTime)} className={styles['clock-icon']} alt="" />
                            </span>
                          </div>
                          <div className={styles['auction-card-info']}>
                            <div className={styles['auction-title']}>{a.title}</div>
                          ...
                          <div className={styles['auction-card-image-container']}>
                            <img src={imgUrl(a.mainImageUrl)} className={styles['auction-card-image']} alt={a.title} />
                          </div>
                          {a.auctionState === 'inProgress' && (
                            <div className={styles['auction-card-actions']}>
                              ...
                              <button className={`${styles['action-button']} ${styles['edit-button']}`}>
                                Edit
                              </button>
                            </div>
                          )}
                ...
                <>
                  <div className={styles['auction-grid']}>{placeholders(6)}</div>
            ...
            {subTab === 'bidding' && (
              bidding.length ? (
                <div className={styles['auction-grid']}>{placeholders((6 - (bidding.length % 6)) % 6)}</div>
              ) : (
                <>
                  <div className={styles['auction-grid']}>{placeholders(6)}</div>
                  <div className={styles['empty-state']}>
            ...
            {subTab === 'won' && (
              won.length ? (
                <div className={styles['auction-grid']}>{placeholders((6 - (won.length % 6)) % 6)}</div>
              ) : (
                <>
                  <div className={styles['auction-grid']}>{placeholders(6)}</div>
                  <div className={styles['empty-state']}>
      ...
      {addOpen && (
        <div className={styles['add-modal-overlay']} onClick={() => setAddOpen(false)}>
          <div className={styles['add-modal-body']} onClick={e => e.stopPropagation()}>
            <h2>Add auction</h2>
